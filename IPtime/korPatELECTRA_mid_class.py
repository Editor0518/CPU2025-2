...
# ===== 데이터 필터링 =====
# 라벨 개수 2개 미만 제거 (단, 'N'은 예외로 유지)
insufficient_labels = [label for label in label_counts.index if label_counts[label] < 2 and label != 'N']
sufficient_labels = [label for label in label_counts.index if label_counts[label] >= 2 or label == 'N']

if insufficient_labels:
    print(f"\n❌ 제외할 라벨 (N 제외): {insufficient_labels}")
...
for epoch in range(1, EPOCHS + 1):
    ...
    if val_loss < best_val_loss:
        ...
        early_stop_count = 0
        torch.save(model.state_dict(), "korpat_electra_mid_classifier.pth")
    else:
        early_stop_count += 1
...
# ===== 평가 =====
model.load_state_dict(torch.load("korpat_electra_mid_classifier.pth", weights_only=True))
